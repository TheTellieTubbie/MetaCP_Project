...
public:
  I(const std::string &host, KS skI, KP pkR) {
    ...
    c = new net::Channel(20334 + 'I');
    this->ni.push_back(groupExp.rndexp(SECURITY_PARAMETER));
    m1 = aenc(pkR, concat(ni, pI));
    message << m1;
    c->send(message.str(), 20334 + 'R', host);
    C m2 = C(c->receive());
    P nr = snd(adec(skI, m2));
    m3 = aenc(pkR, nr);
    message << m3;
    c->send(message.str(), 20334 + 'R', host);
...
public:
  R(const std::string &host, KS skR, KP pkI) {
    ...
    c = new net::Channel(20334 + 'R');
    C m1 = C(c->receive());
    this->nr.push_back(groupExp.rndexp(SECURITY_PARAMETER));
    P ni = fst(adec(skR, m1));
    m2 = aenc(pkI, concat(ni, nr));
    message << m2;
    c->send(message.str(), 20334 + 'I', host);
    C m3 = C(c->receive());
    std::cout << "ni = " << ni << std::endl;
  };
...
void syntax(const char* exename) {
  std::cerr << "Syntax: " << exename << " <g> <p> <pI> <skI> <pkR> <skR> <pkI> <I|R> [remote-host]" << std::endl;
}
